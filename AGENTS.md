...
Status: v0.1. The detection layer (ten deterministic checks) is built and
registered; it installs as a Claude Code plugin (`ai-ready`) or from npm
(`ai-coding-readiness`). The adapt half — the interview, the marked regions,
the one-PR delivery — is not built yet.
