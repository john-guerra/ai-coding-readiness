 ...
 * `writeFile`/`mkdir` do, so a lexical check alone contains nothing on a
 * filesystem where the repo may contain a symlink. `insideRoot` is the first
 * gate; `lib/writer.js` re-asserts it against `realpath()`ed ancestors as it
 * builds the path, which is the gate that actually holds.
 */
