...
async function run(repo) {
  ...
  // deleted it) or unparseable as YAML. Both are the same defect class: a
  // pull-request gate could be hiding in a file we never read, so neither
  // may silently drop out of the count that decides `pass`.
  const unexamined = [];
        ...
        for (const rawLine of cmd.split("\n")) {
          ...
          for (const { pattern, label } of VOLATILE_STEP_PATTERNS) {
            if (pattern.test(line)) {
              hits.push({ path, jobName, label, cmd: line });
            }
          }
        }
      }
  ...
  if (unexamined.length > 0) {
    ...
    return makeFinding({
      ...
      evidence:
        ...
        `could not examine: ${skipped}. A pull-request gate cannot be ruled ` +
