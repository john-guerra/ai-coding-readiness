...
async function run(repo) {
  ...
  const base = {
    ...
    precondition: null,
    // Not auto-fixable, and not temporarily: the remediation moves steps out
    // of an existing workflow's YAML into a scheduled one. That is a rewrite
    // of a repository's CI, which is outside the three things this tool is
    // allowed to do to a file — by decision, not by omission.
    autoFixable: false,
    action: null,
  };
