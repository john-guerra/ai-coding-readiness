...
/**
 * A browser-binary install invocation, up to the next shell separator.
 *
 ...
 *
 * It is stripped from the line rather than disqualifying the line, because
 * the install and the run are routinely one command:
 * `npx playwright install --with-deps && npx playwright test`. Dropping that
 * whole line took the test invocation with it, leaving the job with no
 * browser tier at all — which read as `unknown` alone and, worse, as `pass`
 * beside a properly sharded sibling: the report announcing the tier was
 * sharded while an unsharded job sat unexamined in the same workflow.
 */
const INSTALL_INVOCATION = /\b(?:playwright|cypress)\s+install\b[^&;|]*/g;
...
function browserTierLines(run) {
  return run
    ...
    .map((l) => l.replace(INSTALL_INVOCATION, ""))
    .filter((l) => BROWSER_TIER.test(l));
}
...
function matrixKeyReachesCommand(commands, key) {
  ...
  return new RegExp(`\\$\\{\\{\\s*matrix\\.${escaped}\\s*\\}\\}`).test(
    commands,
  );
}
...
async function run(repo) {
  const base = {
    ...
    // Not auto-fixable, and not temporarily: sharding a job means rewriting an
    // existing workflow's YAML — a matrix, a shard argument, a merge step.
    // Rewriting a repository's CI is outside the three things this tool is
    // allowed to do to a file, by decision rather than by omission.
    autoFixable: false,
    action: null,
  };
