...
async function run(repo) {
  ...
  return makeFinding({
    ...
    fix:
      ...
      "read past.",
    // TEMPORARY: false only until this check carries an `append-lines` action
    // for the missing `.gitignore` entries. `autoFixable` now means "ships
    // something a machine can apply", and this one does not yet. Note that the
    // lockfile half of the fix stays a human step either way — running
    // `npm install` is not one of the three things this tool may do.
    autoFixable: false,
    action: null,
  });
}
