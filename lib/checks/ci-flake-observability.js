...
async function run(repo) {
  ...
  // than a parse. A commented-out `retries` would read as present, and a
  // `retries` key need not sit at the start of its line (e.g. an inline
  // object literal); that is an accepted limitation, stated here rather
  // than hidden.
  if (/\bretries\s*:/.test(text)) {
    return makeFinding({
      ...base,
      status: "pass",
