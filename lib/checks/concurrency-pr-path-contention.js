...
/**
 * Line count, matching `wc -l` — the number of newline characters.
 *
 * `text.split("\n").length` reads the trailing newline almost every file ends
 * with as one more line, so every size this check reported was one higher
 * than the number the maintainer gets from their own shell. A tool whose
 * argument is that your numbers are stale has to agree with `wc -l`.
 *
 * @param {string} text
 * @returns {number}
 */
...
async function run(repo) {
  ...
  if (lists.length === 0) {
    return makeFinding({
      ...base,
      status: "unknown",
      // What was observed is the absence of merge commits, and that is all.
      // A squash-merge or rebase workflow produces it, but so does a shallow
      // clone, a directory that is not a git repository, and a repository
      // whose history simply has not merged anything yet. Naming one of them
      // as the cause asserts something that was never checked.
      evidence:
        "No merge commits were found in this directory, so there is no " +
        "merged-PR history to profile. Possible explanations, none of them " +
        "verified here: a squash-merge or rebase workflow (which leaves no " +
        "merge commits), a shallow clone, or a path that is not a git " +
        "repository.",
      fix: null,
      autoFixable: false,
    });
  ...
  if (lists.length < MIN_MERGE_COMMITS) {
    return makeFinding({
      ...
      status: "unknown",
      evidence:
        `Only ${lists.length} merge commit(s) found; ` +
        `${MIN_MERGE_COMMITS} are needed before a contention share means ` +
        `anything. At this sample size a file touched by a single merge sits ` +
