...
async function run(repo) {
  ...
  return makeFinding({
    ...
    evidence,
    fix: fixes.join("\n\n"),
    // This used to be conditional — true when there was a metadata half to fix
    // and nothing contended that a person had to decide about. That condition
    // described who the remediation was FOR, not whether we could perform it,
    // and `autoFixable` now means the second thing: the finding ships an action
    // a machine applies. The metadata remediation is "adopt changesets or
    // towncrier" — a tool choice, a config, a CI step, and a convention the
    // team has to keep — which is not one of the three things this tool may do
    // to a file. Permanently false, not pending an action.
    autoFixable: false,
    action: null,
  });
}
