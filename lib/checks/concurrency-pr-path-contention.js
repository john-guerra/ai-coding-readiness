...
async function run(repo) {
  ...
  const { source, reason, lists } = await repo.mergedPrFileLists(SAMPLE);
  ...
  const apiNote =
    source === "github-api"
      ? ", read from the GitHub API because this repository leaves no merge commits"
      : "";
  ...
  let evidence = `Contention profile over the last ${lists.length} merged PRs${apiNote}:\n  ${lines.join("\n  ")}`;
  if (attentionText) evidence += `\n\n${attentionText}`;
