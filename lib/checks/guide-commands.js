...
async function run(repo) {
  ...
  return makeFinding({
    ...
    // Not auto-fixable, and not temporarily: the fix is prose inserted into a
    // guide a person wrote, in their section order and their voice. We could
    // paste the command in mechanically; what we cannot do is decide where it
    // belongs in someone else's document, and a Commands section dropped in
    // the wrong place is worse than the sentence telling them to write one.
    autoFixable: false,
    action: null,
  });
}
