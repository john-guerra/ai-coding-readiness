 ...
 *
 * Containment is not paranoia here, it is the audit's read-only guarantee
 * meaning something. Paths reach these accessors from repo-controlled content
 * — an `@../secret.md` import written in a CLAUDE.md is enough — and the
 * report they feed is one the issue template asks people to paste in public.
 * Before this, `readFile` was a bare `join(root, path)`, so that import made
 * the tool read a file outside the directory it was pointed at and quote a
 * line of it verbatim.
 *
 ...
 * inside `/a/root` — a bare `startsWith` says it does.
 *
 * @param {string} root
 * @param {string} p
 */
