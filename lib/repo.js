 ...
 * @param {{files?: Record<string,string>, mergedPrFileLists?: string[][], prListSource?: "merge-commits"|"github-api"|"none", prListReason?: string|null}} data
 * @returns {Repo}
 */
export function createFakeRepo({
  ...
  return {
    ...
    async mergedPrFileLists(n) {
      ...
      return { source: prListSource, reason: resolvedReason, lists };
    },
  };
}
...
function explainGhFailure(err) {
  ...
  if (/git remotes|known GitHub host|not a git repository/i.test(text)) {
    return "no GitHub remote was found, so there are no pull requests to profile";
  }
  return `the GitHub API could not be reached: ${text.trim().split("\n")[0]}`;
}
...
export function createFsRepo(root, { exec = run } = {}) {
  return {
    ...
    async mergedPrFileLists(n) {
      ...
      if (fromGit.length >= n) {
        return { source: "merge-commits", reason: null, lists: fromGit };
      }
      ...
      if (fromApi && fromApi.length > fromGit.length) {
        return { source: "github-api", reason: null, lists: fromApi };
      }
      if (fromGit.length > 0) {
        return { source: "merge-commits", reason: null, lists: fromGit };
      }
