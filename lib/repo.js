 ...
 * disk or the network.
 *
 * Every path handed to `readFile` and `listFiles` is refused unless it stays
 * inside `root` **after symlinks are resolved** — the lexical `insideRoot`
 * first, then the same check again against `realpath()`, exactly as
 * `lib/writer.js` does before a write. A refused path behaves exactly as an
 * absent one (`null` / `[]`) rather than throwing, so no check has to learn a
 * new failure mode and none of them can be talked into reporting on a file the
 * user did not point the tool at.
 *
...
export function createFsRepo(root, { exec = run } = {}) {
  /**
   * The root with symlinks resolved, computed once. Everything below is
   * compared against it rather than against `root`, because a root is
   * frequently a symlink itself (macOS `/tmp` and `/var` both are) and a
   * lexical comparison would then reject every legitimate read.
   * @type {Promise<string|null>|null}
   */
  let realRootOnce = null;
  const realRoot = () => (realRootOnce ??= realpath(root).catch(() => null));

  /**
   * Resolve `path` under the root and re-assert containment against the REAL
   * path, the way `lib/writer.js` does before every write.
   *
   * `insideRoot` alone is lexical, and `readFile`/`readdir` follow symlinks —
   * so a `CLAUDE.md -> /outside/secret.md` link inside the repository made the
   * audit read a file outside the directory it was pointed at and quote a line
   * of it, verbatim, into a report `.github/ISSUE_TEMPLATE/bug.md` asks people
   * to paste in public. The `@../secret.md` traversal was closed a milestone
   * ago; this is the same hole by a different route.
   *
   * Returns null for anything that does not resolve inside the root, INCLUDING
   * a path that does not exist — a refused path behaves exactly as an absent
   * one, so no check has to learn a new failure mode.
   *
   * @param {string} path
   * @returns {Promise<string|null>} the real absolute path, or null
   */
  const resolveInside = async (path) => {
    // Lexical gate first: cheap, and it is the only one that can see the path
    ...
    try {
      ...
      // Missing, dangling, or a component that is not a directory. All of
      // those are "not there", which is what the callers already handle.
      return null;
    }
    return insideRoot(base, real) ? real : null;
  };
  ...
  return {
    ...
    async readFile(path) {
      ...
      const real = await resolveInside(path);
    ...
    async listFiles(dir) {
      // Outside the audited repository is the same as not there.
      const real = await resolveInside(dir);
      if (real === null) return [];
      try {
