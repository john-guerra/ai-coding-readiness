 ...
 *
 * Not fence-aware: marker syntax written in prose (e.g. a README documenting
 * this format inside a code fence) counts the same as a real marker and can
 * trip a false refusal. That is the safe direction to be wrong in — a missed
 * write, not a silent corruption — so it is left as a known tradeoff rather
 * than taught to parse markdown here.
 *
 * @param {string} text
 * @param {string} id
 */
 ...
 *   ambiguous document is not safe to blindly overwrite; or
 * - the matched region's own inner content contains a marker for a
 *   *different* id. `rawMarkerCounts` only counts tokens for the target id,
 *   so an interleave like `begin(g)/AAA/begin(h)/BBB/end(g)/CCC/end(h)`
 *   passes that guard — `findRegion` then swallows h's opening marker (and
 *   everything up to g's end) into g's "inner", and an upsert would delete
 *   it silently while `stripRegions` reports the write as clean. Refusing
...
export function upsertRegion(text, id, inner, version) {
  ...
  if (found) {
    if (MARKER_TOKEN_RE.test(found.inner)) {
      throw new Error(
        `refusing to upsert id ${JSON.stringify(id)}: the matched region's content already contains a marker for a different id`,
      );
    }
